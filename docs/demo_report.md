...
原始 LFW 图像
    ...
    │
    ▼ Phase 2 — 图像分割（ResUNet 深度学习，GrabCut 伪标签弱监督训练）
    │
...
### 4.1 当前方法：ResUNet（深度学习）

**架构**：ResNet-18 编码器（ImageNet 预训练）+ U-Net 解码器，输入 3×112×112，输出 1×112×112 二值掩膜。
...
|-------|-----------|---------|---------|
| 1  | 0.3932 | 0.3432 | 0.8454 |
| 5  | 0.1742 | 0.2036 | 0.8648 |
| 10 | 0.0885 | 0.1820 | 0.8630 |
| **17** | **0.0481** | **0.1909** | **0.8729 ✓** |
| 20 | 0.0455 | 0.1937 | 0.8695 |
...
第二阶段早期测试了四种传统分割方法，以前景像素占比为评估指标：

![图像分割方法对比](figures/demo_segmentation.png)

| 方法 | 前景占比 | IoU vs GrabCut | 特点 |
|------|---------|--------------|------|
| YCrCb 阈值 | 51.5% ±15.7% | — | 椭圆肤色模型；速度最快 |
| GMM 肤色 | 96.4% ±6.0% | — | 数据驱动；覆盖率高（裁剪图几乎全为人脸） |
| GrabCut | 23.6% ±14.7% | (reference) | 图割优化；边界平滑；**用作 ResUNet 伪标签** |
| Watershed | 35.0% ±10.4% | 0.290 | 距离变换 + 分水岭 |
| **ResUNet（当前）** | **27.9%** | **0.807** | **深度学习；边界最优** |

> GrabCut 作为伪标签来源，ResUNet 与其高度一致（Dice=0.827），并产生更平滑的边界；Watershed IoU=0.290 说明传统方法间质量差距显著。
...
### 6.4 为什么 B Naive 与 C v3 结果完全相同？

**这是数学必然，而非巧合。**

余弦相似度的值域严格为 $[-1, 1]$。v3 设置 `L1_HIGH = -1`，判断条件为 `score > -1`：

| 情况 | 发生概率 | 原因 |
|------|---------|------|
| `score > -1`（走 L1） | **≈ 100%** | 两向量完全反向才能得 -1，512-d 特征空间中概率趋近于零 |
| `score < L1_LOW=-2`（拒识） | **0%** | 余弦相似度不可能低于 -1 |
| 走 L2 | **0%** | 以上两条件覆盖所有情况，L2 分支永远不触发 |

因此 v3 的预测逻辑**退化为与 B Naive 完全一致**：

```python
# v3 等价于
pred = cosine_top1(full_face_emb, gallery)   # 就是 B
```

实验数字精确相等（89.11% 全量，92.70% 50样本）正是**代码正确性的自验证**：若 v3 ≠ B，才说明实现存在 bug。这一设计也使得 v3 可以作为"关闭级联"的基准，与 v1/v2 形成干净的对照实验。
...
| 策略 | 50 样本 C | 全量 C | 备注 |
|------|-----------|--------|------|
| **B Naive** | 92.70% | **89.11%** | 参照，全量完成 |
| **C v1** (L1=0.80, replace) | 12.43% | **12.52%** ✅ | 全量完成，阈值过高 |
| **C v2** (L1=0.50, best-of-two) | 47.93% | **—** ❌ | 未运行（预计 ~97 min） |
| **C v3** (L1=−1, 禁用L2) | 92.70% | **89.11%** ✅ | 全量完成，最优策略 |

### 各遮挡类型全量细分

| 遮挡类型 | B Naive | C v1（全量） | C v2（全量） | C v3（全量） |
|---------|---------|------------|------------|------------|
| sunglasses（墨镜） | 88.79% | **8.23%** ✅ | — | **88.79%** ✅ |
| cup（水杯） | 87.85% | **17.45%** ✅ | — | **87.85%** ✅ |
| glasses（眼镜） | 90.69% | **11.89%** ✅ | — | **90.69%** ✅ |
| **整体** | **89.11%** | **12.52%** | **—** | **89.11%** |

> v2 全量根据 50 样本趋势（47.93% vs B 的 92.70%）判断仍远低于 B，故未继续耗费 ~97 min CPU 时间完成全量运行。
