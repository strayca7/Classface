...
4. [第二阶段：图像分割（深度学习 + 传统方法对比）](#四第二阶段图像分割深度学习--传统方法对比)
...
- 图像分割部分**先实现四种传统机器学习方法进行定量对比**，后期重构为 **ResUNet 深度学习方法**（ResNet-18 + U-Net，弱监督伪标签训练）
- 识别流水线（Phase 3–5）在 CPU 上运行；ResUNet 训练支持 CUDA / Apple MPS / CPU 自动检测
...
【第一阶段】预处理与人脸对齐
        ...
        ▼
【第二阶段】图像分割（ResUNet 深度学习为主，传统方法保留对比）
        │  主流程：ResUNet（ResNet-18 + U-Net，GrabCut 伪标签弱监督）
        │  对比基线：YCrCb 阈值 | GMM 肤色分割 | GrabCut | Watershed
        ▼
【第三阶段】基线识别系统验证
...
| 深度学习 | PyTorch + torchvision | ResUNet 训练与推理 |
| 人脸检测/识别 | insightface (buffalo_l) | ArcFace 512-d 特征 |
| 传统机器学习 | scikit-learn | GMM 肤色分割（历史对比）|
| 数值计算 | numpy | 向量化余弦相似度 |
...
对 112×112 预处理人脸图像进行前景分割，输出二值掩膜（0/255）。**主流程采用 ResUNet 深度学习方法**（ResNet-18 编码器 + U-Net 解码器），以 GrabCut 伪标签进行弱监督训练；同时保留四种传统方法作为对比基线，定量验证各方法的特性差异。

> **说明**：当前版本的分割模块作为独立的探索性工作存在，分割掩膜未直接集成进 Phase 3–5 识别流水线。本模块的价值在于：① 覆盖课程"图像分割"相关要求；② 定量对比五种方法的边界质量；③ 为未来掩膜引导的遮挡感知识别提供基础设施。
...
传统分割方法依赖手工特征（肤色阈值、图割能量函数），对光照变化和非标准肤色鲁棒性有限。ResUNet 通过卷积神经网络自动学习多尺度语义特征，在不需要人工标注的情况下，利用 GrabCut 的输出作为**弱监督伪标签**进行训练，实现端到端的前景分割。

#### 4.2.2 网络架构

```
输入 (3×112×112, ImageNet 归一化)
    │
    ▼  Encoder（ResNet-18 预训练）
  [Stem  56²  64ch]─────────────────────────────────────┐
  [Layer1 28²  64ch]────────────────────────────────┐   │
  [Layer2 14²  128ch]──────────────────────────┐   │   │
  [Layer3  7²  256ch]─────────────────────┐   │   │   │
  [Layer4  4²  512ch]                     │   │   │   │
    │                                     │   │   │   │
    ▼  Decoder（双线性上采样 + 跳跃连接）       │   │   │   │
  [Up+concat → Conv×2, 256ch]←────────────┘   │   │   │
  [Up+concat → Conv×2, 128ch]←────────────────┘   │   │
  [Up+concat → Conv×2,  64ch]←────────────────────┘   │
  [Up+concat → Conv×2,  32ch]←────────────────────────┘
    │
    ▼  Head
  Conv(1×1) → Sigmoid → 1×112×112 前景概率图
    │
    ▼  阈值 0.5 → 二值掩膜
```
...
| 模块 | 参数量 | 说明 |
|------|--------|------|
| ResNet-18 编码器 | ~11.2M | ImageNet 预训练，迁移学习 |
| U-Net 解码器 | ~1.9M | 随机初始化，在 LFW 上训练 |
| 总计 | ~13.1M | 轻量级，M3 MPS 可训练 |

#### 4.2.3 训练策略

| 项目 | 设置 |
|------|------|
| 训练数据 | LFW 处理图像 9,164 张（80/20 划分） |
| 标签来源 | GrabCut 伪标签（`data/segmented/grabcut/`，弱监督） |
| 损失函数 | 0.5 × BCE + 0.5 × Dice Loss（应对前景/背景不均衡） |
| 优化器 | Adam，lr=1e-4，CosineAnnealingLR |
| 训练轮数 | 20 epoch，best checkpoint（val Dice 最高）保存 |
| Batch size | 16（MPS/CUDA），CPU 自动降至 4 |
| 数据增强 | 随机水平翻转 + 亮度扰动 ±20 |
| 设备 | Apple M3 MPS，训练耗时约 22 分钟 |
...
|-------|-----------|---------|---------|------|
| 1  | 0.3932 | 0.3432 | 0.8454 | 初始收敛 |
| 5  | 0.1742 | 0.2036 | 0.8648 | 快速下降 |
| 10 | 0.0885 | 0.1820 | 0.8630 | 趋于稳定 |
| 15 | 0.0546 | 0.1899 | 0.8721 | 逼近最优 |
...
### 4.3.3 方法 C：GrabCut 前景分割（同时用作 ResUNet 伪标签）
...
| v2（best-of-two） | 0.50 | 取 L1/L2 最高分 | **47.93%** | **48.35%** | ✅ 已完成 |
...
以下汇总所有已完成的全量评估（1,680 身份，22,452 张遮挡图像）与 50 身份抽样结果。
...
|---------|---------|------------|------------|------------|
| sunglasses | 88.79% | **8.23%** | 40.31% | **88.79%** |
| cup | 87.85% | **17.45%** | 50.21% | **87.85%** |
