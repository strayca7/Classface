...
对 112×112 预处理人脸图像进行前景分割，输出二值掩膜（0/255）。**主流程采用 ResUNet 深度学习方法**（ResNet-18 编码器 + U-Net 解码器），以 GrabCut 伪标签进行弱监督训练；同时保留四种传统方法作为历史对比基线，验证深度学习方案的优越性。
...
### 4.3.1 方法 A：YCrCb 颜色阈值分割（已调整编号，历史保留）
