...
| 组别 | 输入 | 识别策略 | 研究意义 |
|------|------|---------|---------|
| **A — 基线** | 干净图像（无遮挡） | 全脸 ArcFace 直接检索 | 系统上界 |
| **B — 遮挡 Naive** | 合成遮挡图像 | 全脸 ArcFace 直接检索 | 遮挡造成的原始损害 |
...
| 遮挡类型 | B（Naive） | C（级联 v1） | n |
|---------|-----------|------------|---|
| sunglasses（墨镜） | 88.79% | 8.23% | 7,484 |
| cup（水杯） | 87.85% | 17.45% | 7,484 |
| glasses（眼镜） | 90.69% | 11.89% | 7,484 |
...
#### 迭代 1：best-of-two（L1_HIGH=0.5）

降低 L1_HIGH 到 0.5，L2 决策改为"取 L1 与 L2 中置信度更高的预测"。

```python
L1_HIGH = 0.5
# score > 0.5 → L1 直接输出
# score ≤ 0.5 → L2 best-of-two：final = L2 if score_L2 > score_L1 else L1
```

50 身份抽样结果：**B=92.70%，C=48.92%**

分析：有所改善但仍远低于 B，说明 L2 眼周特征在低置信区间表现差。

#### 迭代 2：类型感知级联（cup 用 L2，glasses/sunglasses 用 L1）

```python
EYE_SAFE_TYPES = {"cup"}  # 水杯遮挡嘴部，眼周理论上干净
# cup → L2 best-of-two；glasses/sunglasses → 保留 L1
```
...
| 类型 | B | C（迭代 2） |
|------|---|-----------|
| sunglasses | 94.67% | **94.67%** ✓（= B） |
| cup | 89.94% | **44.97%** ✗（L2 仍然差） |
| glasses | 93.49% | **93.49%** ✓（= B） |

结论：即使是理论上"眼周干净"的水杯类型，L2 眼周裁剪也无法改善结果。

#### 迭代 3：禁用 L2（L1_HIGH=-1，最优策略）

设置 `L1_HIGH=-1`（余弦相似度 ∈ [-1, 1]，所有正常检测的图像满足 score > -1，全部走 L1）：

```python
L1_HIGH = -1  # 禁用 L2 路由；所有可检测图像走 L1 直接输出
```

50 身份抽样结果：**B=92.70%，C=92.70%（= B）**

全量预期：**C ≈ B ≈ 89.11%**（待全量 eval-compare v2 完成确认）

### 7.7 现阶段最优策略

| 版本 | L1_HIGH | 50 样本 C | 全量 C | 状态 |
|------|---------|---------|--------|------|
| v1（原始） | 0.8 | — | **12.52%** | 已完成（阈值错误） |
| v2（best-of-two） | 0.5 | **48.92%** | — | 抽样验证 |
| v3（禁用L2，当前） | -1 | **92.70%** | *待运行* | 代码已就绪 |
...
| 遮挡图像（50 样本） | 级联 v3（L1_HIGH=-1） | **≈92.70%** | ≈ B，最优 |
...
*报告生成时间：2026-05-12*  
*当前项目版本：0.1.0*  
*待完成：eval-compare v2 全量运行（`make eval-compare`，预计 ~137 min）*
